        ...
        if t % PlotFreq == 0:
            ...
            if not makeVid:
                if t == 0:
                    ...
                    ws = (list(model.parameters())[0].data.cpu().numpy())
                    bs = (list(model.parameters())[1].data.cpu().numpy())
                    x_brk_new = torch.from_numpy(np.divide(-1 * bs, np.transpose(ws)[0])).to(device)
                    x_brk_new = x_brk_new.view(-1, 1)
                    movement  = torch.mean(torch.abs(x_brk_new-x_brk))
                    x_brk = x_brk_new
                    brk_moves.append(movement)
