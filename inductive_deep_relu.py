...
H_tot = 20     #Total number of neurons
...
plot_deep_brk = True       #Plot the breakpoints for deeper layers THIS IS SLOW
...
# device = f'cuda:{core}'
tag = f'Tests/bad_init'
getOut = False           #Use this if you want to get the relu states in deeper layers
...
grow_prune = True      #Moves the breakpoints with smallest slope to area with smallest error
# percent = .2            #What percent of the network is pruned per pruning iteration
usecca = False          #Perform CCA on the second to last layer
percent =.015
PruneFreq = int(NumIters/(12)) #Generally the network is pruned for the first half of the training
seed_num = 2019
seed = True
        ...
        if t % PlotFreq == 0:
            ...
            if usecca:
                if t==0:
                    ...
                    model2.to(device)
                    laststr = numH - 2
                    lastHstr = 'hiddenL' if laststr < 0 else f'hiddenH.{laststr}'
                    # lastHstr = 'outputL'
                    firstHook = CCAHook(model2, lastHstr, svd_device="cuda")
                    torch.manual_seed(seed_num)
                    main_hook = firstHook.data(ds, batch_size=len(x), num_workers=0).to(device)
                    model2.eval()
                    with torch.no_grad():
                        model2(main_hook)
                # model2.load_state_dict(())

                hooks.append(CCAHook(model,lastHstr,svd_device = "cuda"))
                torch.manual_seed(seed_num)
                curr_hook = hooks[-1].data(ds,batch_size=len(x),num_workers=0).to(device)
                model.eval()
                with torch.no_grad():
                    model(curr_hook)
                model.train()
                scores.append(hooks[-1].distance(firstHook))
            if not makeVid:
                # ylim(-2, 2)
                # xlim(-5, 5)
                ws = (list(model.parameters())[0].data.cpu().numpy())
            ...
            if makeVid:
                ...
                y_brk = model(x_brk)
                model.train()
                if (activationFunc == 'DeepReLu' and (H_2 == 0 or numH == 0)) or activationFunc != 'DeepReLu':
                ...
                if activationFunc == "DeepReLu" and getOut and plot_deep_brk:
                    ...
                    layerIter = 0
                    tot_bps = H
                    if t == 0: #initializing the axes
                        ax = []
                        for layer_idx in range(len(deep_layers)): #Create a subplot of the layers, with the last one being the output
                            ax.append(plt.subplot2grid((len(deep_layers), 2), (layer_idx, 0)))
                            ax[-1].title.set_text(f'Hidden Layer {layer_idx+2}')
                            ax[-1].set_ylim(-2, 2)
                            ax[-1].set_xlim(-5, 5)

                        ax.append(plt.subplot2grid((len(deep_layers), 2), (0, 1), rowspan=len(deep_layers)))
                        ax[-1].title.set_text(f'Output')
                        ax[-1].set_ylim(-2, 2)
                        ax[-1].set_xlim(-5, 5)
                        tight_layout()
                        subplots_adjust(top=.88)
                        plot(xx, yy, 'g.', label='Train')
                    ...
                    for layer in deep_layers: #layer[0] is preReLu activation layer[1] is postReLu activation
                        ...
                        brk_plot_sub = ax[layerIter - 1].scatter(x_brk_h, y_brk_h, marker='x',
                                                                 c=[cmap4((layerIter + 2) / (numH + 5))],
                                                                 animated=True)
                        imgs.append(brk_plot_sub)
                        brk_proj_h = scatter(x_brk_h, np.zeros(len(x_brk_h)) - (1.7 - .1 * layerIter), marker='|',c = [cmap4((layerIter+2)/(numH + 5))],
                                           animated=True) #also needs to be plotted on the other plot
                        brk_proj_sub = ax[layerIter-1].scatter(x_brk_h, np.zeros(len(x_brk_h)) - (1.7), marker='|',c = [cmap4((layerIter+2)/(numH + 5))],
                                           animated=True)
                        imgs.append(brk_proj_h)
                        imgs.append(brk_proj_sub)

                        piece_text = ax[layerIter - 1].text(2, 1.7, 'pieces = ' + str(bps))
                        imgs.append(piece_text)
                    ...
                    brk_plot_h = scatter(x_brk_h, y_brk_h, marker = 'x',c = [cmap4((layerIter+2)/(numH + 5))],
                                       animated=True)
                    imgs.append(brk_plot_h)
                    brk_plot_sub = ax[layerIter - 1].scatter(x_brk_h, y_brk_h, marker='x', c=[cmap4((layerIter + 2) / (numH + 5))],
                                         animated=True)
                    imgs.append(brk_plot_sub)
                    brk_proj_h = scatter(x_brk_h, np.zeros(len(x_brk_h)) - (1.7 - .1 * layerIter), marker='|',c = [cmap4((layerIter+2)/(numH + 5))],
                                       animated=True)
                    brk_proj_sub = ax[layerIter - 1].scatter(x_brk_h, np.zeros(len(x_brk_h)) - (1.7),
                                                             marker='|', c=[cmap4((layerIter + 2) / (numH + 5))],
                                                             animated=True)
                    imgs.append(brk_proj_h)
                    imgs.append(brk_proj_sub)

                    for layer_idx,layer in enumerate(deep_layers): #These plots go on the other plots
                        for unit in range((layer[0].data.shape[-1])): #Each neuron in the deep layers
                            #Pre ReLU
                            nn_plot_1, = ax[layer_idx].plot(x_test.data.cpu().numpy(), layer[0].data.cpu().numpy()[:, unit], 'r-',
                                              color=(cmap2(1/4 + .1*layerIter)), animated=True,linewidth=.5)
                            #Post_ReLU
                            nn_plot_2, = ax[layer_idx].plot(x_test.data.cpu().numpy(), layer[1].data.cpu().numpy()[:, unit], 'k-',
                    ...
                    tot_piece = text(2, 1.35, 'pieces = ' + str(tot_bps))
        ...
        if grow_prune:
            if (t % PruneFreq == 0) & (t > 1) & ((t < NumIters / 2)):
                if activationFunc == "DeepReLu" and getOut:
                   ...
                   valueadded = torch.mul(valueadded,dict(model.named_parameters())['outputL.weight'].data)
                   ...
                   laststr = numH-2
                   if laststr <0:
                       dict(model.named_parameters())[f'hiddenL.bias'].data += -1 * y_value
                   else:
...
if plot_loss:
    clf()
    semilogy(Time_scale, Loss, '-r',linewidth = 1.0,label = f"H_2 = {H_2}",c=cmap4((3+1)/10))
...
if 'Deep' in activationFunc:
    ...
    fig.suptitle(f'{activationFunc} Net ({D_in},{H}{netString},{D_out}) trained for {NumIters} Iters w/ LR={learning_rate} w/ Init = {initFunc}')
    tight_layout()
    subplots_adjust(top=.88)
else:
